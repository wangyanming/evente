...
public class DateUtils {
	...
	public static void main(String[] args) throws Exception {
		System.out.println(dateStampToDay(System.currentTimeMillis()));
	}
	 ...
	 * @return
	 */
	public static List<String> getAllMinute(String date) {
		List<String> list = new ArrayList<>();
		int hour;
		int minute = 60;
		int currentMinute;
		Calendar cal = Calendar.getInstance();
		if (date.equals(dateStampToDay(System.currentTimeMillis()))) {
			...
			for (int i = 0; i <= hour; i++) {
				for (int j = 0; j < minute; j++) {
					if (i < 10 && j < 10) {
						list.add(date + " 0" + i + ":0" + j + ":00");
					} else if (i < 10 && j >= 10) {
						list.add(date + " 0" + i + ":" + j + ":00");
					} else if (i >= 10 && j < 10) {
						list.add(date + " " + i + ":0" + j + ":00");
					} else if (i == hour && j == currentMinute) {
						break;
					} else {
						list.add(date + " " + i + ":" + j + ":00");
					}
				}
			}
		} else {
	...
	public static List<String> getAllDay(String startDate, String endDate) {
		...
		list.add(endDate + " 00:00:00");// 把结束时间加入集合
		Collections.sort(list);
		return list;
	}
